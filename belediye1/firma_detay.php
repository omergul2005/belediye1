...
// BASİTLEŞTİRİLMİŞ: Taksit takvimi oluşturma fonksiyonu (Gecikme faizi YOK)
function generateInstallmentCalendar($firma, $odemeler) {
    if (!$firma) return [];
    ...
    // Ödenen taksitleri tarihe göre sırala
    $odenen_taksitler = [];
    foreach ($odemeler as $odeme) {
        $odenen_taksitler[] = $odeme['odeme_tarihi'];
    }
    sort($odenen_taksitler);
    ...
    for ($i = 1; $i <= $firma['taksit_sayisi']; $i++) {
        ...
        // Ödenen taksitler için ödeme tarihi buls
        if ($i <= count($odenen_taksitler)) {
            $odeme_tarihi = $odenen_taksitler[$i - 1];
            $durum = 'odendi';
            $durum_renk = '#28a745';
            $durum_text = 'Ödendi';
            
            // Gecikme gün hesabı (sadece görüntü için)
            $odeme_date = new DateTime($odeme_tarihi);
            if ($odeme_date > $vade_tarihi) {
        ...
        } else if ($today > $gecikme_tarihi) {
            $durum = 'gecikme';
            $durum_renk = '#dc3545';
            $durum_text = 'Gecikme';
            $gecikme_gun = $gecikme_tarihi->diff($today)->days;
        } else if ($today > $vade_tarihi) {
            $durum = 'vade_gecti';
            $durum_renk = '#fd7e14';
            $durum_text = 'Vadesi Geçti';
...
// Taksit takvimiçini oluştur
$taksit_takvimi = generateInstallmentCalendar($firma, $odemeler);
?>
<!DOCTYPE html>
<html lang="tr">
<head>
    ...
    <style>
        * {
        ...
        .status-bekliyor {
            background: linear-gradient(135deg, rgba(108, 117, 125, 0.2), rgba(134, 142, 150, 0.1));
            color: #495057;
        ...
        .status-vade_gecti {
            background: linear-gradient(135deg, rgba(253, 126, 20, 0.2), rgba(243, 156, 18, 0.1));
            color: #856404;
            border: 2px solid rgba(253, 126, 20, 0.3);
        }
        
        .status-tamamlandi {
            background: linear-gradient(135deg, rgba(0, 123, 255, 0.2), rgba(52, 152, 219, 0.1));
            color: #004085;
    ...
    <div class="container">
        ...
        <div class="content">
            ...
            <?php if ($firma): ?>
                ...
                <!-- BASİTLEŞTİRİLMİŞ: Taksit Takvimi (Gecikme faizi YOK) -->
                <?php if (!empty($taksit_takvimi)): ?>
                <div class="taksit-container">
                    <div class="taksit-header">
                        <h3>📅 Taksit Takvimi</h3>
                        <span><?php echo $firma['taksit_sayisi']; ?> Taksit</span>
                    </div>
                    ...
                    <table class="taksit-table">
                        ...
                        <tbody>
                            <?php foreach ($taksit_takvimi as $taksit): ?>
                            <tr>
                                <td><strong><?php echo $taksit['taksit_no']; ?>. Taksit</strong></td>
                                <td><?php echo $taksit['vade_tarihi']; ?></td>
                                <td class="amount">₺<?php echo number_format($taksit['tutar'], 0, ',', '.'); ?></td>
                                <td>
                                    <span class="status-badge status-<?php echo $taksit['durum']; ?>">
                                        <?php echo $taksit['durum_text']; ?>
                                    </span>
                                </td>
                                <td>
                                    <?php if ($taksit['odeme_tarihi']): ?>
                                        <strong style="color: #27ae60;"><?php echo date('d.m.Y', strtotime($taksit['odeme_tarihi'])); ?></strong>
                                    <?php else: ?>
                                        <span style="color: #6c757d;">-</span>
                                    <?php endif; ?>
                                </td>
                                <td>
                                    <?php if ($taksit['gecikme_gun'] > 0): ?>
                                        <span style="color: #dc3545; font-weight: bold;"><?php echo $taksit['gecikme_gun']; ?> gün</span>
                                    <?php else: ?>
                                        <span style="color: #6c757d;">-</span>
                                    <?php endif; ?>
                                </td>
